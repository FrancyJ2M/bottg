...
def deltabot_init(bot: DeltaBot) -> None:
    ...
    bot.account.set_config("displayname","Bridge_Tg")
    ...
    MAX_AUTO_CHATS = bot.get('MAX_AUTO_CHATS') or 20
    ...
    bot.commands.register(name = "/sms" ,func = async_login_code, admin = True)
       ...
       for d in all_chats:
           ...
           if True:
              ...
              if str(d.id) in chatdb[message.get_sender_contact().addr]:
                 comando = '<br><a href="mailto:'+bot_addr+'?body=/remove_'+str(d.id)+'">❌ Desvilcular</a>&nbsp; &nbsp; &nbsp;<a href="mailto:?body=/link2_'+str(d.id)+'">📎 Vincular</a>'
              else:
                 comando = '<br><a href="mailto:'+bot_addr+'?body=/load_'+str(d.id)+'">✅ Cargar</a>&nbsp; &nbsp; &nbsp;<a href="mailto:?body=/link2_'+str(d.id)+'">📎 Vincular</a>'
              try:
                  if is_img:
...
def async_chat_news(bot, payload, replies, message):
    """Muestra la lista de los chats con mensajes no leidos...
    Puede pasar a sus chats privados: /news private
        Tambien puede cargarlos con sus fotos de perfil: /news img
        Para obtener todos sus chats: /news full.
        Puede combinarlos de esta forma:
            /news full img"""
    loop.run_until_complete(chat_news(bot, payload, replies, message))


def link_to(bot, payload, replies, message):
    """Une un chat de  Telegram con uno de Delta.
    Es decir lo q reciba en el chat de Telegram se enviara tambien al de Delta q ah asociado"""
    if payload:
...
def async_chat_info(bot, payload, replies, message):
    """Obtien informacion respondiendo un mensaje con /info."""
...
def async_pin_messages(bot, message, replies):
    """Debe proporcionar su pin para acceder a los chats privados: /pin
    """
...
def async_forward_message(bot, message, replies, payload):
    """Reenvia msg de un chat a otro.
    Con el id del msg y el id del destinatario.
    Ej: /forward 234 me
    """
...
def list_chats(replies, message, payload):
    """Muestra la lista de los chats vinculados"""
    if message.get_sender_contact().addr not in logindb:
...
async def add_auto_chats(bot, replies, message):
    """Activa la entrada automatica de msg en el chat"""
...
def remove_chat(bot, payload, replies, message):
    """Remueve los chat vinculados con el puente.
    Ej: Remover todos sus chats /remove all 
    Remover solo uno con su id: /remove -10023456789"""
    target = None
...
def logout_tg(bot, payload, replies, message):
    """Cerrar sesion de su cuenta en el bot"""
...
def async_login_num(payload, replies, message):
    """Iniciar sesion en Telegram con su numero telefonico.
    Ej: /login +5312345678"""
...
def async_login_2fa(bot, payload, replies, message):
    """Confirmar sesion si tiene el 2FA activo: /pass PASSWORD"""
...
def async_login_session(bot, payload, replies, message):
    """Iniciar sesion con su token de cuenta
    Ej: /token abigtexthashloginusingintelethonlibrary..."""
...
def async_updater(bot, payload, replies, message):
    """Cargar sus msg de Telegram.
    Ej: /load
    Puede cargar sus chat privados: /load #privates
    O con su nombre un chat especifico: /load delta chat
    Tambiem se cargaran con su id espec铆fico
    Ej: /load_-1001327680159"""
...
def async_click_button(bot, message, replies, payload):
    """Hacer click en un botton.Estos apareceran automaticos ya q llevan un id para q el bot los accione"""
...
def async_react_button(bot, message, replies, payload):
    """Selecionar reaccion al msg.Puede saber las reacciones disponibles enviando /react
    Ej: /react ❤"""
...
def async_load_chat_messages(bot, message, replies, payload):
    """Cargar msg de de Telegram en el chat.Usando el id carga al chat,el msg correspondiente...con esto puede cargar msg anteriores 
    Cargar solo 5 msg : /more 5
    Cargar msg #10: /more -10
    Cargar el ultimo msg: /more last"""
...
def async_down_chat_messages(bot, message, replies, payload):
    """Descarga archivos enviados al chat de Telegram esto se utiliza para evitar saturar su bandeja de entrada de tantos MB asi usted decide q bajar del chat.
    Esto se utiliza con el id del msg"""
    loop.run_until_complete(load_chat_messages(bot=bot, message=message, replies=Replies, payload=payload, dc_contact = message.get_sender_contact().addr, dc_id = message.chat.id, is_auto = False))

def async_comment_chat_messages(bot, message, replies, payload):
    """Carga los comentarios de un post en Telegram con el id correspindiente del msg"""
...
def async_echo_filter(bot, message, replies):
    """Hola..!\n\nSoy un bot puente entre Telegram/Delta.Para una forma mas economica de acceder a sus chats\n«Revisa mi Texto de Firma 🤖🕯»️"""
    global authorize_url
...
def async_send_cmd(bot, message, replies, payload):
    """Enviar cmd a los chats de Telegram.
    Ej: /b /help"""
...
def async_inline_cmd(bot, message, replies, payload):
    """Comando de busqueda para los bots de telegrama en linea: /inline gif dog"""
    loop.run_until_complete(inline_cmd(bot, message, replies, payload))

def async_inmore_cmd(bot, message, replies, payload):
    """Caragar mas resultado de la ultima busqueda realizada"""
    loop.run_until_complete(inline_cmd(bot, message, replies, payload))

def async_indown_cmd(bot, message, replies, payload):
    """Descargar resultado de la busqueda realizada 
    Ej: /indown 5"""
    loop.run_until_complete(inline_cmd(bot, message, replies, payload))

def async_inclick_cmd(bot, message, replies, payload):
    """Ejecutar resultado de la busqueda del bot en linia.
    Ej: /inclick 5"""
    loop.run_until_complete(inline_cmd(bot, message, replies, payload))
...
def async_search_chats(bot, message, replies, payload):
    """Busqueda de chats publicos en Telegram: /search delta chat"""
...
def async_join_chats(bot, message, replies, payload):
    """Empezar chat con usuarios o un grupo/canal
    Ej: /join usernamegroup
    o /join https://t.me/joinchat/invitehashtoprivatechat"""
...
def async_preview_chats(bot, payload, replies, message):
    """Obtiene vista previa del chat  /preview usuario"""
...
def create_comment_chat(bot, message, replies, payload):
    """Crea un chat donde puede comentar el msg dado por id."""
    target = get_tg_id(message.chat, bot)
    if DBXTOKEN or DATABASE_URL:
